...
if (isset($_GET['delete_user'])) {
  ...
  if ($existing_count > 0) {
    ...
    $update_sql = "DELETE FROM tbl_admin WHERE user_id = ?";
    $update_stmt = $conn->prepare($update_sql);
    $update_stmt->bind_param("i", $user_id);
    if ($update_stmt->execute()) {
      $msg_success = " Removed Successfully";
...
if (isset($_POST['add_new_team'])) {
  ...
  if ($existing_count > 0) {
    ...
    if (!empty($_POST['p_manager'])) {
      ...
      $update_sql = "UPDATE tbl_admin SET p_team = ?, user_role = 3 WHERE user_id = ?";
    } else {
      $update_sql = "UPDATE tbl_admin SET p_team = ?, user_role = 3 WHERE user_id = ?";
    }
    $update_stmt = $conn->prepare($update_sql);
                            ...
                            <form role="form" action="" method="post" autocomplete="off">
                                <div class="form-horizontal">
                                    ...
                                    <div class="form-group">
                                        ...
                                        <div class="col">
                                            <input type="password" placeholder="Enter Password" name="em_password" class="form-control input-custom" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                                        <label class="control-label col-sm-4">Confirm Password</label>
                                        <div class="col">
                                            <input type="password" placeholder="Confirm Password" name="em_confirm_password" class="form-control input-custom" required>
                                        </div>
                                    </div>
                                    <div class="form-group">
                ...
                <form role="form" action="" method="post" autocomplete="off">
                  <div class="form-horizontal">
                    ...
                    <div class="form-group">
                      ...
                      ?>
                      <select class="form-control" name="p_manager" id="p_manager">
                  ...
                  ?>
                    <tr>
                      ...
                      <td><a title="Update Employee" href="update-employee.php?admin_id=<?php echo $row['user_id']; ?>"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-pencil-square" viewBox="0 0 16 16">
                        ...
                        <a title="Delete" href="?delete_user=<?php echo $row['user_id']; ?>" onclick=" return confirm('Are you sure ?');"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
