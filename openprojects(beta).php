    ...
    <table class="table text-center table-sm w-100 " id="open-project-table">
      ...
      <thead class="fixedheader" style="" class="">
        ...
        <tr class="text-light align-middle font-weight-normal text-center fixedheader" style="height: 3.5rem; ">
          ...
          <th width="10%" class="fixedheader align-middle font-weight-normal text-center">
            Id</th>

          <th width="18%" class="fixedheader align-middle font-weight-normal text-center">
            Title</th>
          <th width="" class="fixedheader align-middle font-weight-normal text-center">Customer
          </th>



          <th width="" class="fixedheader align-middle font-weight-normal text-center">
            Team</th>
          <th width="6%" class="fixedheader align-middle font-weight-normal text-center">
            PM</th>
          <th width="6%" style="" class="fixedheader align-middle font-weight-normal text-center">
            Engineer</th>
          <th width="6%" class="fixedheader align-middle font-weight-normal text-center">
            Checker</th>
          <th width="" class="fixedheader align-middle font-weight-normal text-center">
          ...
          <th width="6%" class="fixedheader align-middle font-weight-normal text-center">
            ECD</th>
          <th width="13%" style="" class="fixedheader align-middle font-weight-normal text-center">
            Comments</th>
    ...
    <div class="modal-content">
      ...
      <div class="modal-body">
        <form action="" method="post">
          <input type="" id="follow_project_id" name="project_id">
          <input type="" id="follow_table_id" name="table_id">
  ...
  fetch('api.php') // Update with the correct API path
    ...
    `;
        ...
        tr.innerHTML = `
          ...
          <div style="${pidBorder}; display:flex; align-items: center; font-size: 0.9rem; justify-content: center; height:1.5rem; margin-left: 1rem; width:3.2rem; text-align: center; border-radius: 5px; background-color: ${urgencyColor};  color: ${textColor};"> 	${row.revision_project_id || row.project_id}</br>
          </div>
                      ...
                      ${(row.p_team && row.p_team.toLowerCase() === 'count') ? `
        <div>
            <span class="badge badge-pill badge-danger m-0">Quick Project</span>
        </div>` : ''}
          ...
          <td class="align-middle my-0 py-0" ><p hidden>${row.state || row.sub_state || 'N/A'}</p>${row.p_team || 'N/A'}</td>
...
${(userRole == 1 || userRole == 3) && (row.assign_to_id !== null && row.assign_to_id !== '') && row.assign_to_status != 1 ? (row.assign_status != 1 || row.assign_to_id !== 'null' ? `
              ...
              class="custom-blue-border custom-blue-font assign-btn" 
                            ...
                            data-bs-project-id="${row.project_id}" 
                           data-bs-assign_to_id="${row.assign_to_id}"
                            style=" "
      ...
      // document.querySelectorAll(".comments-form").forEach((item) => {
      //   item.addEventListener("input", (e) => {

      //     let targetTextbox = e.target.id
      //     document.getElementById(`save-btn${targetTextbox}`).style.display = "block"
    ...
    const separator = "-".repeat(46); // Creates a long line of dashes (adjust length as needed)
    ...
    return comments
        ...
        .join("\n\n"); // Join with double new lines for spacing
}
      ...
      $(document).ready(function() {
        ...
        var dataTable = $('#open-project-table').DataTable({
          paging: true,
          pageLength: 50,
          lengthMenu: [20, 40, 100, 150],
          searching: true,
          ordering: true,
          stateSave: true,
          ...
          stateLoadCallback: function(settings) {
            var state = JSON.parse(localStorage.getItem('dataTableState'));

            setTimeout(function() {
              // Check if filters were applied before reload
              if (state && state.search && state.search.search) {
                $('#customSearchInput').val(state.search.search);
              }
              toggleClearButton(); // Ensure the button is updated after restoring the state
            }, 300);
        ...
        $('#customPageInput').val(dataTable.page.len());

        // Clear search and filters
        $('#clear-search').on('click', function() {
          $('#customSearchInput').val('');
          $('#project-id-filter, #engineer-filter, #customer-id-filter, #team-filter,#state-filter').val('');
          $('#project-id-filter, #engineer-filter, #customer-id-filter, #team-filter,#state-filter').trigger('change');

          dataTable.search('').columns().search('').draw();

          // Clear DataTable saved state
          dataTable.state.clear();
          dataTable.ajax.reload(); // If using AJAX

          $(this).hide(); // Hide the button after clearing filters
        });

        function toggleClearButton() {
          if ($('#customSearchInput').val().trim() ||
            $('#project-id-filter').val().trim() ||
            $('#engineer-filter').val().trim() ||
            $('#customer-id-filter').val().trim() ||
            $('#team-filter').val().trim() ||
            $('#state-filter').val().trim()) {
            $('#clear-search').fadeIn(300); // Smooth fade-in over 200ms
          } else {
            $('#clear-search').fadeOut(300); // Smooth fade-out over 200ms
          }
        }

        // Call this function whenever a filter changes
        $('#customSearchInput, #project-id-filter, #engineer-filter, #customer-id-filter, #team-filter,#state-filter').on('input change', toggleClearButton);

        // Call it on page load to ensure the button starts hidden if no filters are applied
        toggleClearButton();
        ...
        document.getElementById("apply-filter").addEventListener("click", function() {
          ...
          var selectedStateFilter = document.getElementById('state-filter').value;

          console.log(selectedStateFilter)
          ...
          dataTable.column(5).search(selectedEngineerFilter).draw();

          // Apply the customer id filter to the DataTable (assuming customer_id is on column 3, change index if needed)
          dataTable.column(2).search(selectedCustomerIdFilter).draw();

          // Apply the team filter to the DataTable (assuming team is on column 4, change index if needed)
          dataTable.column(3).search(selectedTeamFilter).draw();

          dataTable.column(3).search(selectedStateFilter).draw();
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update_followup'])) {
    ...
    $new_followup_entry = "\n\n" . $current_date . " --- " . $comments . " --- " . $manager_name;
