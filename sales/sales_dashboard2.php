      ...
      <body>
          <div class='container'>
              ...
              <div class='content'>
                  ...
                  <form action='http://localhost/app.csaengineering.com.au/sales/api.php' method='get'>
...
if (isset($_POST['acceptEnquiry'])) {
  ...
  $enquiry_acknowledged = $_POST['acknowledgementMethod'];
  $status = '25';
  $sql = "UPDATE enquiry_sales SET priority =? , enquiry_status = ?, enquiry_acknowledged = ? WHERE id = ?";
  $stmt = $conn->prepare($sql);
  $stmt->bind_param('sssi', $priority, $status, $enquiry_acknowledged, $enquiryId);
  $stmt->execute();
  $stmt->close();
}
  ...
  <div class="container-fluid  mt-md-5">
    ...
    <div class="container">
      <ul class="nav nav-pills">
        <li class="nav-item"><a class="nav-link" data-toggle="modal" href="" data-target="#add_enquiry">Create New Enquiry</a></li>
        <li class="nav-item"><a class="nav-link active" data-toggle="pill" href="#home">New Enquiries</a></li>
        <li class="nav-item"><a class="nav-link" data-toggle="pill" href="#menu1">High Priority Enquiries</a></li>
        <li class="nav-item"><a class="nav-link" data-toggle="pill" href="#menu2">Medium Priority Enquiries</a></li>
        <li class="nav-item"><a class="nav-link" data-toggle="pill" href="#menu3">Low Priority Enquiries</a></li>

      </ul>
      <div class="tab-content mt-5">
        <?php
        $tabs = [
          ['id' => 'home', 'label' => 'New Enquiries', 'condition' => '1=1'],
          ['id' => 'menu1', 'label' => 'High Priority Enquiries', 'condition' => 'e.priority = "High"'],
          ['id' => 'menu2', 'label' => 'Medium Priority Enquiries', 'condition' => 'e.priority = "Medium"'],
          ['id' => 'menu3', 'label' => 'Low Priority Enquiries', 'condition' => 'e.priority = "Low"']
        ];

        foreach ($tabs as $tab) {
          echo '<div id="' . $tab['id'] . '" class="tab-pane fade ' . ($tab['id'] === 'home' ? 'show active' : '') . '">';
          echo '<h3>' . $tab['label'] . '</h3>';
          echo '<table id="myTable' . $tab['id'] . '" class="display"><thead><tr><th></th></tr></thead><tbody>';

          $sql = "SELECT e.*, c.*
                FROM enquiry_sales e
                LEFT JOIN contacts c ON e.contact_id = c.contact_id
                WHERE {$tab['condition']} AND enquiry_status != 100
                ORDER BY e.flag DESC;";

          $result = $conn->query($sql);

          if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
              if ($row['enquiry_status'] == "0" || $tab['id'] !== 'home') {
                echo '<tr><td>';
                echo '<a href="" class="text-decoration-none" data-toggle="modal" data-enquiryId="' . $row['id'] . '" data-status="' . $row['enquiry_status'] . '" data-target="#action" data-folderId="' . $row['folderId'] . '">';
                echo '<div class="card mt-2"><div>';
                echo '<div class="d-flex justify-content-around mt-3 p-3">';
                echo '<div>';
                echo '<medium>Enquiry id <span class="text-primary"># ' . $row['id'] . '</span></medium><br>';
                echo '<medium>Customer Name: <span class="text-info font-weight-bold"> ' . $row['customer_name'] .  $row['potential_customer'] . '</span></medium><br>';
                echo '<medium>Comments: <span class="text-info font-weight-bold"> ' . $row['comments'] .  '</span></medium>';
                echo '</div>';
                echo '<div>';

                echo '<medium class="text-success font-weight-bold">' . $row['enquiry_name'] . '</medium><br>';
                echo '<medium>Last Updated: <span class="text-warning"> ' . $row['last_updated'] . '</span></medium>';
                echo '</div>';

                echo '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-flag-fill text-danger" viewBox="0 0 16 16">';
                if ($row['flag'] == 1) {
                  echo '<path d="M14.778.085A.5.5 0 0 1 15 .5V8a.5.5 0 0 1-.314.464L14.5 8l.186.464-.003.001-.006.003-.023.009a12 12 0 0 1-.397.15c-.264.095-.631.223-1.047.35-.816.252-1.879.523-2.71.523-.847 0-1.548-.28-2.158-.525l-.028-.01C7.68 8.71 7.14 8.5 6.5 8.5c-.7 0-1.638.23-2.437.477A20 20 0 0 0 3 9.342V15.5a.5.5 0 0 1-1 0V.5a.5.5 0 0 1 1 0v.282c.226-.079.496-.17.79-.26C4.606.272 5.67 0 6.5 0c.84 0 1.524.277 2.121.519l.043.018C9.286.788 9.828 1 10.5 1c.7 0 1.638-.23 2.437-.477a20 20 0 0 0 1.349-.476l.019-.007.004-.002h.001"/></svg>';
                }
                echo '</div>';
                ...
                echo '<div class="step ' . ($row['enquiry_status'] >= 25 ? 'accepted' : '') . '"> <span>Enquiry Acknowledged</span> </div>';
                echo '<div class="step ' . ($row['enquiry_status'] >= 50 ? 'assigned' : '') . '"> <span>Quotation Assigned</span> </div>';
                echo '<div class="step ' . ($row['enquiry_status'] >= 60 ? 'ready' : '') . '"> <span>Quotation Ready</span> </div>';
                echo '<div class="step ' . ($row['enquiry_status'] >= 75 ? 'sent' : '') . '"> <span>Quotation Sent</span> </div>';
                echo '<div class="step ' . ($row['enquiry_status'] == 100 ? 'completed' : '') . '"> <span>RAE Project</span> </div>';
                echo '</div>';
                echo '</div>';
                echo '</div>';
              }
      ...
      <div class="modal-content">
        ...
        <form method="POST">
          ...
          <div class="modal-footer">
            <button type="button" class="btn btn-secondary" data-dismiss="modal">Cancel</button>
            <input type="text" name="enquiryId" id="enquiryId" hidden readonly>
            <button class="btn btn-success" type="submit" name="acceptEnquiry">Change</button>
          </div>
        </form>
      </div>
    </div>
