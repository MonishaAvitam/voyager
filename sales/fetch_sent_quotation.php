...
if (!empty($revised_qt)) {
    ...
    $query = "SELECT * FROM potential_project_sent_quotation WHERE sales_id = '$sales_id' AND revised_qt = '$revised_qt'";
} else {
    // Fetch the latest revised quotation for the given sales_id
    $query = "SELECT * FROM potential_project_sent_quotation WHERE sales_id = '$sales_id'";
}
