...
?>
<?php
// Get office_id from URL
$office_id = isset($_GET['office_id']) ? intval($_GET['office_id']) : 0;
$office_name = "N/A";

if ($office_id > 0) {
    $stmt = $conn->prepare("SELECT office_name FROM office WHERE id = ?");
...
<div class="mx-auto position-relative" style="width: 97.5%;">
    <div class="d-sm-flex align-items-center justify-content-between">
        <div>
            <h3>All Potential Clients</h3>
            <h4 class="text-muted">Office: <?= htmlspecialchars($office_name) ?></h4>
        </div>
        <div>
...
<script>
    ...
    let apiUrl = `./fetch_potential_customer.php?office_id=${officeId}`; // Fetch by office only
    ...
    document.querySelectorAll('#leadsTab .nav-link').forEach(tab => {
        tab.addEventListener('shown.bs.tab', function (e) {
            ...
            let apiUrl = `./fetch_potential_customer.php?office_id=${officeId}`; // Always fetch by office
    ...
    <div class="d-flex justify-content-center align-items-center gap-3">
        ...
        <!-- Convert -->
        <span class="quotation-btn extra-action-btn" role="button" 
            data-bs-id="${row.id}"  
            data-bs-toggle="modal" data-bs-target="#convertModal"
            data-tippy-content="Convert potential Clients into CSA clients.">
            <svg class="w-6 h-6 text-gray-800 dark:text-white" aria-hidden="true" 
                xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" 
                viewBox="0 0 24 24">
        ...
        <span class="quotation-btn delete-record-btn" role="button" 
            ...
            <svg class="trash-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" fill="none" stroke="currentColor" viewBox="0 0 24 24">
