...
include '../include/viewportSidebar.php';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($project_id && $num_subprojects > 0) {
        $stmt = $conn->prepare("INSERT INTO subprojects (project_id, subproject_name, subproject_details, assign_to, subproject_status) VALUES (?, ?, ?, ?, ?)");
        if ($stmt) {
            for ($i = 1; $i <= $num_subprojects; $i++) {
                $subproject_name = "N/A"; // Auto-generated name
                $assign_to = "Unassigned"; // Default assignment

                $stmt->bind_param("issss", $project_id, $subproject_name, $subproject_details, $assign_to, $subproject_status);
                $stmt->execute();
            }
            echo "<script>
            Swal({
                ...
                window.location.href = 'test.php?message=success';
            });
        </script>";
            $stmt->close();
        } else {
        ...
        $custom_message = "No subprojects were created because the number provided is invalid appleeeeee.";
    }
}
?>
...
<div id="wrapper" style="">
    ...
    <!-- Sidebar -->
    <!-- <div style="width: 100px; height: 3.2rem; position:fixed; top: 0.2rem; left: 0.2rem; z-index: 10; background: lightgray;">

        <img class=" " src="./csalogo.webp" alt="logo" style="width: 100px; height: auto;  z-index: 10;" />
        </div> -->

    <ul class="d-flex justify-content-center viewport-sidebar" id=""
        ...
        <div class="mx-auto" style="width: 2rem;">
            ...
            <div class="w-full ">
                ...
                <!-- Modal -->
                <!-- <div class="modal fade" id="enter-search-keyword-modal" tabindex="-1" aria-labelledby="exampleModalLabel" aria-hidden="true">
  <div class="modal-dialog">
    <div class="modal-content">
      ...
      <div class="modal-footer">
        ...
        <button type="button" class="btn btn-primary">Save changes</button>
                ...
                <li>
                    <div data-bs-toggle="tooltip" data-toggle="modal" data-target="#add_project" data-placement="right"
                        data-bs-custom-class="custom-tooltip-sidebar" data-tippy-content="Add New Project"
            ...
            <div class=" " style="margin-top: 11.2rem;">
                <li>
                    <div class="w-full" style="margin-top: 1.65rem;"><a href="../welcome.php" data-bs-toggle="tooltip"
                ...
                <li>
                    <div class="w-full " style="margin-top: 1.65rem;"><a href="#" data-toggle="modal" data-target="#logoutModal"
                            data-bs-toggle="tooltip" data-placement="right"
    ...
    <div id="content-wrapper" class="d-flex flex-column">
        ...
        <div class="modal fade text-dark" id="indicator-modal" tabindex="-1"
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body text-dark">
                        ...
                        <div class="d-flex flex-wrap justify-content-center mt-5">
                            ...
                            <div class="d-flex flex-column align-items-center mx-2">
                                <div
                                    style="border: 2px solid gray; width: 5rem; height: 1.4rem; border-radius: 50%;">
                                </div>
        ...
        <div class="table-container " style="max-height: 100vh; overflow-y: auto;">
            <table class="table text-center table-sm table-container" id="sub-projects">
                ...
                <thead class="fixedheader" style="" class="">
            ...
            fetch('../api.php') // Update with the correct API path
                ...
                .then(data => {
                    ...
                    data.forEach(row => {
                        ...
                        <td class="align-middle  py-0 my-0" data-bs-toggle='tooltip' data-placement='bottom' data-tippy-content="<b>Customer Name</b>:${row.customer_name ?row.customer_name:'N/A'}<br><b>Contact Name</b>:${row.contact_name ? row.contact_name :'N/A'}<br><b>Email</b>:${row.contact_email ?row.contact_email :'N/A'}<br><b>Phone</b>: ${row.contact_phone_number ? row.contact_phone_number : 'N/A'}" style="position:relative; ">${row.customer_name ? row.customer_name.split(' ')[0] : 'N/A'}</td>
                        <td class="align-middle py-0 my-0" style=" ">${row.project_manager ? row.project_manager : 'N/A'}</td>
                        ...
                        <td class="subproject-data py-0 my-0" data-reopen-status=${row.reopen_status} data-revision-project-id="${row.revision_project_id}" data-project-id="${row.project_id}" data-project-name="${row.project_name}" data-projectdetails="${row.project_details}" data-urgency="${row.urgency}" data-start_date="${row.start_date}" data-end_date="${row.end_date}" data-assign-to-id="${row.assign_to_id}" data-comments="${row.comments}" data-team="${row.p_team}" >Loading...</td>
                    `;
                ...
                .then(subprojectData => {
                    document.querySelectorAll('.subproject-data').forEach(td => {
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 5px;">
                            ...
                            data-bs-toggle="tooltip" data-placement="bottom" data-tippy-content="<b>${sub.subproject_name}</b></br><b>Assigned to:</b> ${sub.assign_to || sub.assign_to_id}</br><b>ETD: </b>${sub.sub_end_date}</br><b>Project Type: </b>${sub.outsourced == 1 ? 'Outsourced' : 'Internal'}<br/><b>Comments: <b/>${sub.comments ?sub.comments:'N/A' }" class=" my-1 subproject-anime card shadow-sm subprojects-card" style="padding: 0.3rem; cursor:pointer; border-left: 5px solid ${sub.urgency}; width: 7.2rem; height: 1.8rem; font-size: 0.8rem; border-radius: ${sub.outsourced == 1 ? '50%' : '6px'}">
                            ...
                            <div class="d-flex">
                                ...
                                <span class='ps-2'>${sub.subproject_name.length > 10 ? sub.subproject_name.slice(0, 10) + '...' : sub.subproject_name} </span>
                            </div>
                            </div>
                        </div>
                                `).join('')}
                    ...
                    $(document).ready(function() {
                        ...
                        $('#filterModal form').on('submit', function(e) {
                            ...
                            if (personName !== "") {
                                table.column(2).search(personName);
                            }
        ...
        <div class="modal fade" id="info-modal" tabindex="-1" aria-labelledby="info-modalLabel" aria-hidden="true">
            <div class="modal-dialog modal-xl">
                <form method="POST" action="update_subproject.php" name="submitForm" id="submitForm">
                    <div class="modal-content">
                        <div class="modal-header">
                            <h5 class="modal-title" id="info-modalLabel">Information</h5>
                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                        </div>
                        <div class="modal-body mx-auto" style="width:90%;">
                            ...
                            <table class="table table-responsive table-borderless">
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" id="projectName" class="form-control" name="projectName" required value="Existing Project Name" />
                                    </td>
                                </tr>
                                <tr>
                                    <td class="text-left text-primary">Description:</td>
                                    <td>
                                        <textarea style="width: 35rem;" class="form-control" name="projectDetails" required>Existing project description</textarea>
                                    </td>
                                </tr>
                                <tr>
                                    <td class="text-left text-primary">Status:</td>
                                    <td>
                                        <select id="urgencySelect" class="form-control" name="urgency" onchange="updateUrgencyColor(this)">
                                            <option value="red" style="background:red; color:white;">Very Urgent</option>
                                            <option value="orange" style="background:orange; color:white;">Urgent</option>
                                            <option value="white" style="background:white; color:black;">Waiting</option>
                                            <option value="green" style="background:green; color:white;">In Progress</option>
                                            <option value="navy" style="background:navy; color:white;">Completed</option>
                                            <option value="purple" style="background:purple; color:white;">Closed</option>
                                            <option value="yellow" style="background:yellow; color:black;">On Hold</option>
                                        </select>
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <input type="date" class="form-control" name="startDate" value="2025-01-01" readonly />
                                    </td>
                                </tr>
                                <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <input type="text" class="form-control" name="comments" value="Existing comments here" required />
                                    </td>
                                </tr>
                                <tr>
                                    ...
                                    <td>
                                        <div class="d-flex justify-content-start">
                                            <button type="button" class="btn btn-primary btn-sm industrial-btn" onclick="window.open('https://drive.google.com/drive/u/0/folders/0AMBoRd5BFG7XUk9PVA', '_blank')" style="display: none;">
                                                Open Industrial Team Shared Drive
                                            </button>
                                            <button type="button" class="btn btn-primary btn-sm building-btn" onclick="window.open('https://drive.google.com/drive/u/0/folders/0AE4knKSOSdZOUk9PVA', '_blank')" style="display: none;">
                            ...
                            <div class="d-flex justify-content-between my-3">
                                ...
                                <div> Total: <span id="totalSubprojects" class="text-primary font-weight-bold"></span></div>
                            </div>
                            <div class="info-table-container table-responsive" style="max-height: 200px">
                                <table class="mx-auto mt-3 table" style="width:95%" id="subproject-modal">
                                    <thead class="">
                                        <tr class="" style="background: #133E87; color: white;">
                            ...
                            <div class="mt-3 text-left font-weight-bold">Payment Status: </div>
                            <div class="table-responsive info-table-container mt-3" style="max-height: 200px">
                                <table class="m-auto table" style="width:95%" id="payment_status">
                                    <thead class="">
                                        <tr class="" style="background: #133E87; color: white;">
                                            <th class=" fixed text-white text-center">Invoice Number</th>
                                            <th class=" fixed text-white text-center">Service Date </th>
                                            <th class=" fixed text-white text-center">Due Date </th>
                                            <th class=" fixed text-white text-center">Payment Status</th>
                                            <th class=" fixed text-white text-center" width="55%">Comments</th>
                                        </tr>
                                    </thead>
                                    <tbody>
            ...
            document.addEventListener("DOMContentLoaded", function() {
                document.addEventListener('click', function(event) {
                    if (event.target.closest('.info-icon')) {
                        ...
                        const revision = icon.getAttribute('data-revision');
                        ...
                        document.querySelector('#info-modal input[name="comments"]').value = comments || '';

                        // Select buttons safely
                        ...
                        if (industrialButton && buildingButton) {
                            if (team === 'Industrial') {
                                industrialButton.style.display = 'inline-block';
                        ...
                        fetch(`../invoice_api.php?project_id=${projectId}`)
                            ...
                            .then(financeData => {
                                ...
                                // Filter finance data that match the selected projectId
                                const matchedInvoices = financeData.filter(invoice => invoice.project_id == projectId);

                                // Print in console
                                console.log(`Project ID: ${projectId} has ${matchedInvoices.length} matched invoices.`);

                                // Clear the table body before inserting new rows
                                const tbody = document.querySelector('#payment_status tbody');
                                tbody.innerHTML = ''; // Clear previous rows

                                // Append invoice details to the table
                ...
                <tr>
                    ...
                    <td class="text-center">${invoice.payment_status || 'N/A'}</td>
        ...
        <div class="modal fade" id="subprojectModal" tabindex="-1" aria-labelledby="subprojectModalLabel" aria-hidden="true">
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        <form id="updateSubprojectForm" action="update_subproject.php" method="POST">
                            ...
                            <label class="control-label " for="outsourced-yes">Project Type:</label>
                            <div>
                                <!-- Internal Project Type -->
                                <div class="form-group text-left">
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input mt-1" type="radio" name="outsourced" id="outsourced-no" value="0">
                                        <label class="form-check-label d-flex" for="outsourced-no">
                                            Internal
                                            <div class="ml-2 mt-1" style="border: 2px solid gray; width: 3.6rem; height: 1rem;"></div>
                                        </label>
                                    </div>

                                    <!-- Outsourced Project Type -->
                                    <div class="form-check form-check-inline">
                                        <input class="form-check-input mt-1" type="radio" name="outsourced" id="outsourced-yes" value="1">
                                ...
                                <div class="mb-3 text-left">
                                    ...
                                    <input type="text" class="form-control fw-bold" id="subprojectName" name="subproject_name" required>
                                </div>
                                <div class="mb-3 text-left">
                                    <label for="subprojectDetails" class="form-label">Subproject Details:</label>
                                    <textarea class="form-control fw-bold" id="subprojectDetails" required name="subproject_details" rows="3" placeholder="Enter subproject details"></textarea>
                                </div>
                                <div class="mb-3 text-left">
                                    <label for="assign_to" required class="form-label service_provider_label" id="service_provider_label"></label>
                                    <select class="form-control assign_to fw-bold" name="assign_to_id_for_internal" required id="assign_to_id">
                                ...
                                <div class="mb-3 text-left">
                                    <input type="text" name="assign_to_id" class="form-control fw-bold" placeholder="Service provider name" id="service_provider_name" style="display: none;">
                                </div>

                                <div class="mb-3 text-left">
                                    <label for="estimateHours" class="form-label">Estimated Hours:</label>
                                    <input type="number" value="" class="form-control fw-bold" id="estimateHours" required name="sub_EPT" placeholder="Enter estimated hours">
                                </div>
                                <div class="mb-3 text-left">
                                    <label for="estimateHours" class="form-label">Estimated date:</label>
                                    <input type="date" value="" class="form-control fw-bold" id="sub_end_date" required name="sub_end_date" placeholder="Enter estimated date">
                                </div>
                                <div class="form-group text-left">
                                    <label class="control-label" required for="project_status">Status:</label>
                                    <div class="form-group">
                                        <select class="form-control fw-bold" id="urgency_value" style="background-color: red;" name="urgency_status" required>
                                            <option style="background: red; color: white;" value="red">Very Urgent</option>
                                            <option style="background: orange; color: white;" value="orange">Urgent</option>
                                            <option style="background: white;" value="white">Waiting </option>
                                            <option style="background: green; color: white;" value="green">In Progress</option>
                                            <option style="background: navy; color: white;" value="navy">Completed</option>
                                            <option style="background: purple; color: white;" value="purple">Closed</option>
                                        </select>
                                    </div>
                                </div>
                                <div class="mb-3 text-left">
                                    <label for="comments" class="form-label">Comments:</label>
                                    <textarea type="text" class="form-control fw-bold" id="comments" name="comments" value=""></textarea>
                                </div>
                                <div class="mb-3 text-left Payable_amount">
                                    <label for="Payable_amount" class="form-label">Enter Payable Amount:</label>
                                    <input type="number" class="form-control fw-bold" id="Payable_amount" name="quoted_cost" value="">
                                </div>
                            </div>
                            <div class="modal-footer d-flex justify-content-between">
                                <div>
                                    <a href="#"
                                        class="delete_subproject"
                                        id="delete_subproject"
                                        title="Delete Subproject"
                                        onclick="openDeletePopup(this)"
                                        data-project-id="123"
                                        data-subproject-id="456"
                                        data-subproject-status="Active"
                                ...
                                <div>
                                    <button type="button" class="btn btn-secondary btn-sm bg-gradient" data-dismiss="modal">Close</button>
                                    <button type="submit" name="save_subprojects" class="btn btn-success btn-sm bg-gradient">Save changes</button>
                                </div>
                            </div>
                        </form>
                    </div>
        ...
        <script>
            $(document).on('click', '.subproject-anime', function(event) {
                if (event.target.closest('.subproject-anime')) {
                    ...
                    document.getElementById('assign_to_id').value = assignToID;
                    ...
                    if (OutSourced === '1') {
                        ...
                        document.getElementById('service_provider_label').textContent = "Service Provider Name:"; // Updated label
                        ...
                        document.getElementById('service_provider_label').textContent = "Assign to:"; // Reset label
        ...
        <!-- Modal Template -->
        <div class="modal fade" id="addSubprojectModal" tabindex="-1" role="dialog" aria-labelledby="addSubprojectModal" aria-hidden="true">
                    ...
                    <form action="" method="POST">
                        <div class="modal-body">
                            ...
                            <div class="mb-3 text-left">
                                ...
                                <input type="number" class="form-control" id="num_subprojects" name="num_subprojects" required>
                            </div>
                        </div>
                        <div class="modal-footer">
