...
<script>
  ...
  $(document).ready(function () {
    $('#dataTable').DataTable({
      ...
      "info": false,
      "sorting": false,          // Disable "Showing X of Y entries" info
    });
  });
</script>
  ...
  <div class="card shadow mb-4">
    ...
    <div class="card-header py-3 d-flex w-100 justify-content-between align-items-center">
      ...
      <div class="d-flex align-items-center">
        <?php
        if (!empty($_GET['search']) || !empty($_GET['team']) || !empty($_GET['customer']) || !empty($_GET['perPage']) || !empty($_GET['sort']) || !empty($_GET['dir'])):
          ?>
          <a href="<?php echo strtok($_SERVER['PHP_SELF'], '?'); ?>" class="btn btn-sm btn-danger ml-2">
            Clear All Filters
          </a>
        <?php endif; ?>
      ...
      <div class="table-responsive p-3">
        ...
        <table class="table shadow-lg table-bordered table-sm" id="dataTable" name="dataTable"
          ...
          <thead style="height:4.5rem;">
            <tr class="bg-primary text-light my-2 text-center">
              ...
              <th scope="col" class="align-middle" width="8%" style="font-weight:200;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'project_id', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'project_id' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Project No.
                </a>
              </th>
              ...
              <th scope="col" class="align-middle" width="11%" style="font-weight:400;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'project_name', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'project_name' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Project Title
                </a>
              </th>
              ...
              <th scope="col" class="align-middle fw-normal" width="13%" style="font-weight:500;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'customer_name', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'customer_name' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Customer Name
                </a>
              </th>

              <th scope="col" class="align-middle fw-lighter" width="10%" style="font-weight:500;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'p_team', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'p_team' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Team
                </a>
              </th>
              ...
              <th scope="col" class="align-middle fw-light" width="10%" style="font-weight:500;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'price', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'price' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Amount
                </a>
              </th>
              ...
              <th scope="col" class="align-middle" width="20%" style="font-weight:500;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'comments', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'comments' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Comments
                </a>
              </th>
              ...
              <th scope="col" class="align-middle" width="10%" style="font-weight:500;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'service_date', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'service_date' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Service Date
                </a>
              </th>
              ...
              <th scope="col" class="align-middle" width="10%" style="font-weight:500;">
                <a href="?<?php echo http_build_query(array_merge($_GET, ['sort' => 'last_modified_date', 'dir' => (isset($_GET['dir']) && $_GET['sort'] == 'last_modified_date' && $_GET['dir'] == 'asc') ? 'desc' : 'asc'])); ?>"
                  class="text-light">
                  Last Modified
                </a>
              </th>
          ...
          <tbody style="background: #dddddd70;">
            ...
            $team = isset($_GET['team']) ? addslashes($_GET['team']) : '';
            $customer = isset($_GET['customer']) ? addslashes($_GET['customer']) : '';
            ...
            if (!empty($searchTerm)) {
              $whereConditions[] = "(project_id LIKE '%$searchTermSafe%' OR 
            ...
            if (!empty($team)) {
              $whereConditions[] = "p_team = '$team'";
            }

            if (!empty($customer)) {
              $whereConditions[] = "customer_name = '$customer'";
            }
            ...
            $sql .= " ORDER BY $orderColumn $orderDir LIMIT $perPage OFFSET $offset";
            ...
            if (!empty($searchTerm)) {
              $countWhereConditions[] = "(project_id LIKE '%$searchTermSafe%' OR 
                               project_name LIKE '%$searchTermSafe%' OR 
                               p_team LIKE '%$searchTermSafe%' OR 
                               customer_name LIKE '%$searchTermSafe%' OR 
                               comments LIKE '%$searchTermSafe%')";
            ...
            if (!empty($team)) {
              $countWhereConditions[] = "p_team = '$team'";
            }

            if (!empty($customer)) {
              $countWhereConditions[] = "customer_name = '$customer'";
            }
        ...
        <div class="d-flex justify-content-between align-items-center mb-3">
          ...
          <ul class="pagination mb-0">
            ...
            if ($page > 1): ?>
              <li class="page-item">
                <a class="page-link" href="?page=<?= $page - 1 ?><?= $querySeparator . $filterQuery ?>">Previous</a>
              </li>
            <?php endif; ?>
            ...
            <?php for ($i = max(2, $page - 2); $i <= min($totalPages - 1, $page + 2); $i++): ?>
              <li class="page-item <?= $i == $page ? 'active' : '' ?>">
                <a class="page-link" href="?page=<?= $i ?><?= $querySeparator . $filterQuery ?>"><?= $i ?></a>
              </li>
            <?php endfor; ?>
            ...
            <?php if ($page < $totalPages): ?>
              <li class="page-item">
                <a class="page-link" href="?page=<?= $page + 1 ?><?= $querySeparator . $filterQuery ?>">Next</a>
              </li>
              <li class="page-item">
                <a class="page-link" href="?page=<?= $totalPages ?><?= $querySeparator . $filterQuery ?>">Last</a>
              </li>
            <?php endif; ?>
          </ul>
        </div>
...
<script>
  ...
  function filterTableByTeam() {
    ...
    for (var i = 0; i < rows.length; i++) {
      var teamCell = rows[i].cells[4]; // 5th column (0-based index 4)
      if (teamCell) {
