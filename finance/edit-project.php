...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["update_project"])) {
    ...
    $customer_id = trim($_POST['customer_id_name']); // This is actually customer_id from form
    $state = $_POST['state'];
    ...
    // 🔹 Fetch customer_name based on customer_id
    $customer_name = null;
    $sqlCustomer = "SELECT customer_name FROM contacts WHERE contact_id = ?";
    if ($stmtCustomer = $conn->prepare($sqlCustomer)) {
        $stmtCustomer->bind_param("s", $customer_id);
        $stmtCustomer->execute();
        $stmtCustomer->bind_result($customer_name);
        $stmtCustomer->fetch();
        $stmtCustomer->close();
    }
    ...
    if (!isset($_SESSION['error_message'])) {
        ...
        if ($stmtProjects = $conn->prepare($sqlProjects)) {
            $stmtProjects->bind_param(
                ...
                $links,
                $project_status,
                $customer_id,   // store actual customer_id in projects
                $task_id
            );

            if ($stmtProjects->execute()) {
                // ✅ Update finance table too
                $sqlFinance = "UPDATE csa_finance_invoiced 
                               SET project_title = ?, 
                                   customer_name = ? 
                               WHERE project_id = ?";
                if ($stmtFinance = $conn->prepare($sqlFinance)) {
                    $stmtFinance->bind_param("sss", $new_project_name, $customer_name, $task_id);
                    $stmtFinance->execute();
                    $stmtFinance->close();
                }
                            ...
                            <form action="" method="post">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                             ...
                                             WHERE c.contact_id = '$task_id';";
                                                    ...
                                                    while ($row = $info->fetch(PDO::FETCH_ASSOC)) {
                                                        ...
                                                        <tr>
                                                            <td>Description</td>
                                                            <td>
                                                                <textarea name="project_details" class="form-control"
                                                                    style="resize: vertical; height:200px"
                                                                    required><?php echo $row['project_details']; ?></textarea>
                                                            </td>
                                                        </tr>
                                                        <tr>
                                                            <td>Customer ID</td>
                                                            <td>
                                                                ...
                                                                <select class="form-control" name="customer_id_name"
                                                                    id="customer_id_name" required>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <div class="form-group">
                                                                    <div>
                                                                        <select class="form-control" name="p_team"
                                                                            id="p_team" required>
                                                                            <option value="Building Team" <?php echo (strcasecmp($row['p_team'], "Building Team") == 0) ? 'selected' : ''; ?>>Building
                                                                                Team</option>
                                                                            <option value="Industrial Team" <?php echo (strcasecmp($row['p_team'], "Industrial Team") == 0 || strcasecmp($row['p_team'], "industrial") == 0 || strcasecmp($row['p_team'], "Industrial") == 0) ? 'selected' : ''; ?>>Industrial Team
                                                                            </option>
                                                                ...
                                                                <div class="form-group">
                                                                    <div>
                                                                        <select class="form-control" name="state" id="state"
                                                                            ...
                                                                            <Option value="QLD" <?php echo (strcasecmp($row['state'], "QLD") == 0) ? 'selected' : ''; ?>>QLD (Queensland)
                                                                            </Option>
                                                                            <Option value="NSW" <?php echo (strcasecmp($row['state'], "NSW") == 0) ? 'selected' : ''; ?>>NSW (New South Wales)
                                                                            </Option>
                                                                            <Option value="WA" <?php echo (strcasecmp($row['state'], "WA") == 0) ? 'selected' : ''; ?>>WA (Western Australia)
                                                                            </Option>
                                                                            <Option value="SA" <?php echo (strcasecmp($row['state'], "SA") == 0) ? 'selected' : ''; ?>>SA (South Australia)
                                                                            </Option>
                                                                            <Option value="VICTORIA" <?php echo (strcasecmp($row['state'], "VICTORIA") == 0) ? 'selected' : ''; ?>>VICTORIA</Option>
                                                        ...
                                                        <tr>
                                                            <td>Project links</td>
                                                            <td>
                                                                <textarea name="links" placeholder="Folder Links"
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td>
                                                                <input type="text" name="estimate_EPT" id="estimate_EPT"
                                                                    class="form-control" value="<?php echo $row['EPT'] ?>"
                                                                    required>
                                                            </td>
                                                        </tr>
                                                            ...
                                                            <td>
                                                                ...
                                                                <select class="form-control" id="project_status"
                                                                    ...
                                                                    <?php if ($row['urgency'] == 'white'): ?>
                                                                        <option value="white" selected>Don't Start the Project
                                                                        </option>
                                                                    <?php else: ?>
                                                                    ...
                                                                    <?php if ($row['urgency'] == 'green'): ?>
                                                                        <option value="green" selected>Ready to Start the
                                                                            Project</option>
                                                                    <?php else: ?>
                                                                        <option value="green">Ready to Start the Project
                                                                        </option>
                                                                    <?php endif; ?>

                                                                    <?php if ($row['urgency'] == 'purple'): ?>
                                                                        <option value="purple" selected>Mark as Completed
                                                                        </option>
                                                                    <?php else: ?>
                                        ...
                                        <div class="form-group">
                                            <div class="col-sm-3">
                                                <a class="btn btn-secondary btn-xs" href="javascript:void(0);"
                                                    onclick="window.location.href = sessionStorage.getItem('lastVisitedURL') || document.referrer ;">Go
                                                    Back</a>
