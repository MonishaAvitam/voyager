...
<div class="container-fluid">
    ...
    <div class="card shadow mb-4">
        ...
        <div class="container-fluid">
            <div class="d-sm-flex align-items-center justify-content-between mt-4">
                <!-- Left Side (Title) -->
                <h1 class="h3 mb-0 text-gray-800">Paid Invoices</h1>
        ...
        <div class="card shadow mb-4" style="border:none">
            ...
            <?php
            // ✅ Initialize $perPage safely (default = 10 if not set)
            $perPage = isset($_GET['perPage']) ? (int) $_GET['perPage'] : 10;
            ?>

            <div class="card-body">
                <div class="table-responsive p-3">
                    <table class=" shadow-lg table-bordered table-sm" id="dataTable" name="dataTable"
            ...
            <div class="table-responsive p-3">
                <table class="table shadow-lg table-bordered table-sm" name="dataTable"
                    style="border: 1px solid #dddddd70">
                    <thead style="height:4.5rem;">
                        <tr class="bg-success text-light my-2 text-center">
                            ...
                            <th scope="col" class="align-middle" width="8%" style="font-weight:200;">
                            ...
                            <th scope="col" hidden class="align-middle fw-normal" width="13%" style="font-weight:500;">
                                vendor_id</th>
                            <th scope="col" class="align-middle fw-normal" width="13%" style="font-weight:500;">
                                <a class="text-light" style="text-decoration:none;" href="?<?php echo getFilterQueryString(); ?>&sort=v.company_name&dir=<?php
                                   echo getNextDir('v.company_name', $currentSort, $currentDir); ?>">
                                    Vendor Name
                                </a>
                            </th>
                            <th scope="col" class="align-middle fw-normal" width="13%" style="font-weight:500;">
                                <a class="text-light" style="text-decoration:none;" href="?<?php echo getFilterQueryString(); ?>&sort=s.name&dir=<?php
                                   echo getNextDir('s.name', $currentSort, $currentDir); ?>">
                                    Services
                                </a>
                            </th>
                            <th scope="col" class="align-middle fw-light" width="10%" hidden style="font-weight:500;">
                                service_id</th>
                            <!-- <th scope="col" class="align-middle fw-light" width="10%" hidden style="font-weight:500;">
                                service_id</th> -->
                            ...
                            <th scope="col" class="align-middle" width="10%" style="font-weight:500;">
                                <a class="text-light" style="text-decoration:none;" href="?<?php echo getFilterQueryString(); ?>&sort=ui.timestamp&dir=<?php
                                   echo getNextDir('ui.timestamp', $currentSort, $currentDir); ?>">
                                    Last Update
                                </a>
                            </th>
        ...
        ui.comments LIKE '%$escapedSearchTerm%'
                      ...
                      SELECT id FROM services WHERE name LIKE '%$service%'
                        ...
                        $totalRow = $totalResult->fetch_assoc(); // ✅ Correct
                        ...
                        while ($row = $info->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            <tr>
                                <td class="project_id pl-2"><?php echo $row['project_id']; ?></td>
                                <td class="invoice_no"><?php echo $row['invoice_no']; ?></td>
                                <td hidden class="vendor_id"><?php echo $row['vendor_id']; ?></td>
                                <!-- Add these fields in your table -->
                                <td class="vendor_id"><?php echo $row['company_name']; ?></td>
                                <!-- Add these fields in your table -->
    ...
    ?>
                                ...
                                <td class="invoice_date"><?php echo $row['invoice_date']; ?></td>
                                <td class="invoice_date"><?php echo date('Y-m-d', strtotime($row['timestamp'])); ?></td>
                                <td>
                                    <div class="d-flex flex-nowrap">
                                        ...
                                        <button class="btn btn-info btn-sm edit-btn text-nowrap" data-bs-toggle="modal"
                                            data-bs-target="#editModal" data-project_id="<?php echo $row['project_id']; ?>"
                                        ...
                                        <form method="post">
                                            <button type="submit"
                                                class="btn btn-danger btn-sm danger-status-btn text-nowrap" name="delete"
            ...
            e.target.textContent = "View More";
                ...
                if (isset($_POST["edit"])) {
                    ...
                    foreach ($check_queries as $table => $query) {
                        ...
                        $table_name = ($table === 'ready_to_pay') ? 'Ready to Pay' : (($table === 'unpaidinvoices') ? 'Unpaid Invoice' : ucfirst(str_replace('_', ' ', $table)));
                    ...
                    // Check if the invoice_no and project_id already exist in paidinvoices
                    $check_query = "SELECT id FROM paidinvoices WHERE invoice_no = ? AND project_id = ? AND id != ?";
                    ...
                    if ($stmt) {
                        ...
                        // Execute the update
                        if ($stmt->execute()) {
                            echo "<script>alert('Invoice updated successfully!'); window.location.href='paidinvoices.php';</script>";
                        } else {
                    ...
                    document.addEventListener("DOMContentLoaded", () => {
                        ...
                        document.querySelectorAll(".change-status-btn").forEach(button => {
                            button.addEventListener("click", function () {
                                ...
                                const invoiceDate = row.querySelector(".invoice_date").textContent.trim(); // Assuming you have this class
...
if (isset($_POST['updateStatus'])) {
    ...
    $dates_sql = "SELECT booked_date, received_date FROM paidinvoices WHERE project_id = ? AND invoice_no = ?";
    ...
    $booked_date = $dates_row['booked_date'] ?? null;
    $received_date = $dates_row['received_date'] ?? null;
    $dates_stmt->close();

    // Input validation
    if (empty($project_id) || empty($invoice_no) || empty($status) || empty($vendor_id) || empty($comments) || empty($service_id) || empty($invoice_date) || (!isset($amount) && $amount !== '0' && $amount !== 0)) {
    ...
    switch ($status) {
        ...
        case 'unpaid':
            ...
            break;
        default:
            die('<script>alert("Invalid status selected.");</script>');
    }
    ...
    try {
        ...
        $tables = ['ready_to_pay', 'paidinvoices', 'unpaidinvoices'];
        foreach ($tables as $delete_table) {
            $delete_sql = "DELETE FROM $delete_table WHERE project_id = ? AND invoice_no = ?";
        ...
        $insert_stmt->bind_param("isssssdss", $project_id, $vendor_id, $invoice_no, $comments, $service_id_json, $invoice_date, $amount, $booked_date, $received_date);
        ...
        echo "<script>
                ...
                window.location.href = 'paidinvoices.php';
              </script>";
        exit;
...
if (isset($_POST['delete'])) {
    ...
    $sql = "DELETE FROM paidinvoices WHERE project_id = ? AND invoice_no = ?";
