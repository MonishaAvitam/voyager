...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['selected_employees']) && is_array($_POST['selected_employees'])) {
        ...
        if (!empty($selectedEmployeeIds)) {
            ...
            p.expenses
        FROM 
            csa_finance_employee_info e
        LEFT JOIN 
            csa_finance_payslip p ON e.Employee_id = p.Employee_id
        WHERE 
            e.Employee_id IN (";
$sql .= rtrim(str_repeat('?,', count($selectedEmployeeIds)), ',') . ")";
            ...
            if ($result->num_rows > 0) {
                ...
                $spreadsheet->setActiveSheetIndex(0)
                    ...
                    ->setCellValue('H1', 'Expenses')
                    ->setCellValue('I1', 'Date of Joining')
                ...
                while ($employee = $result->fetch_assoc()) {
                    ...
                    $spreadsheet->getActiveSheet()->setCellValue('H' . $row, $employee['expenses']);
                    $spreadsheet->getActiveSheet()->setCellValue('I' . $row, $employee['doj']);
                    $spreadsheet->getActiveSheet()->setCellValue('J' . $row, $employee['department']);
                    $spreadsheet->getActiveSheet()->setCellValue('K' . $row, $employee['designation']);
